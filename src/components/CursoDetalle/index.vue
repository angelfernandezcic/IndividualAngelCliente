<template>
    <div id="CursoDetalle">
        <div class="container-fluid">
          ...
          <div class="col-xs-12">  
			...
			<form v-on:submit.prevent="">
			    <v-layout row wrap>
						...
						<v-flex xs6>
							<v-select v-bind:disabled="!isEditable" v-bind:items="itemsRamas" v-model="cursoFiltrado.Rama" label="Seleccione la rama" dark item-value="text" required></v-select>
						</v-flex>
					</v-layout>
					<v-layout row wrap>
						<v-flex xs6>
						...
						<v-flex xs6>
							<v-select v-bind:disabled="!isEditable" v-if="cursoFiltrado.Rama === 'Letras'" v-bind:items="itemsCarrerasLetras" v-model="cursoFiltrado.Carrera" label="Seleccione la carrera" dark item-value="text" required></v-select>
							<v-select v-bind:disabled="!isEditable" v-if="cursoFiltrado.Rama === 'Ciencias'" v-bind:items="itemsCarrerasCiencias" v-model="cursoFiltrado.Carrera" label="Seleccione la carrera" dark item-value="text" required></v-select>
						</v-flex>
					</v-layout>
					<v-layout row>
          <v-flex xs4>
          ...
          <v-flex xs8>
          	<v-text-field v-bind:disabled="!isEditable" v-model="cursoFiltrado.Anyo" :rules="[rules.cursoFiltrado.Anyo]" name="anyoscarrera" label="Años de carrera" type="number" dark required></v-text-field>
          </v-flex>
        </v-layout>
                <v-layout row>
          <v-flex xs4>
